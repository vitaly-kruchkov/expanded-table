import type { JsonValue, TableRow } from "./table.types";
...
export function getRows(): TableRow[] {
  return rows;
}

export function updateRow(id: string, data: Record<string, unknown>) {
  const row = rows.find((r) => r.id === id);
  if (!row) throw new Error("Row not found");
  row.cells = data as Record<string, JsonValue>;
}
